%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Análise Dinâmica de um Pêndulo Livre - Retrato de Fase
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Pêndulo Livre
% condições iniciais  
ci = [-3*pi -2*pi/3 3*pi/2 -2*pi -pi -pi/2 0 pi/2 pi 2*pi 3*pi/2 2*pi/3 3*pi];
vi = [-3 -2.5 -2  -1.5 -1  -0.5  0  0.5  1  1.5  2 2.5 3];
...
for k = 2:N
  ...
  % Cálculo das constantes
	k1a = dottheta(k-1);
	k1b = -(g/l)*sin(theta(k-1)); % ddotheta(i-1)

	k2a = dottheta(k-1) + dt*0.5*k1b; 
	k2b = -(g/l)*sin(theta(k-1) + dt*0.5*k1a);

	k3a = dottheta(k-1) + dt*0.5*k2b;
	k3b =  -(g/l)*sin(theta(k-1) +dt*0.5*k2a);

	k4a = dottheta(k-1) + dt*k3b;
	k4b =  -(g/l)*sin(theta(k-1) + dt*k3a);
  ...
  % aproximação de ddottheta(i)
  ddottheta(k) = -(g/l)*sin(theta(k));
